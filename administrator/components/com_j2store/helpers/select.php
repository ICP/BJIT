...
class J2StoreHelperSelect {
	...
	public static function getOptionTypesList($name, $id, $item) {
		...
		foreach ( $types as $type_key => $typeitems ) {
			...
			$groups [$type_key] ['text'] = JText::_ ( 'J2STORE_OPTION_OPTGROUP_LABEL_' . JString::strtoupper ( $type_key ) );
			$groups [$type_key] ['items'] = array ();
			foreach ( $typeitems as $type ) {
				$groups [$type_key] ['items'] [] = JHtml::_ ( 'select.option', $type, JText::_ ( 'J2STORE_' . JString::strtoupper ( $type ) ) );
			}
		}
