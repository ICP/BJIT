...
$(function () {
    ...
    var Menu = {
        hide: function () {
            ...
            $("#menu").stop().animate({'right': '-75%'}, function () {
                $("#menu").hide(1);
            });
        },
        show: function () {
            $("#menu").stop().show(1).animate({'right': '0'}, function() {
                $('body').addClass('menu-open');
            });
        }
    };
    $(document).on('click', '[data-target="#menu"]', function (e) {
        if ($("#menu").is(':visible'))
            Menu.hide();
        else
            Menu.show();
    ...
    $(document).on('click', function (e) {
        if ($("body").hasClass('_xs')) {
            if ($("#menu").is(':visible') && $(e.target).closest("#menu").length === 0 && $(e.target).closest('[data-target="#menu"]').length === 0) {
                Menu.hide();
                e.preventDefault();
                return false;
            }
...
//                carouselConfig.centerMode = true;
    ...
    var headerHeight = $("#header").height() - $("#menu").height();
