	...
	public function getPriceRanges($items){
		...
		if(count($items )) {
			foreach($items as $item){
				...
				//now round the price
				$roundRange = F0FModel::getTmpInstance('Producttags','J2StoreModel')->_priceRound($range, $params->get( 'list_price_round_digit', '100' ), true);
				//get the lowest price
				$roundPriceLow = F0FModel::getTmpInstance('Producttags','J2StoreModel')->_priceRound( $priceLow, $params->get( 'list_price_round_digit', '100' ) );
			}
		}
