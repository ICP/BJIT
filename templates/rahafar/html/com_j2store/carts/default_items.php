					...
					<td>
						...
						<?php if($this->params->get('show_thumb_cart', 1) && !empty($thumb_image)): ?>
							<span class="cart-thumb-image">
								<img alt="<?php echo $item->orderitem_name; ?>" src="<?php echo JURI::base() . $thumb_image; ?>" >
							</span>
						<?php endif; ?>
					  ...
					  <?php if($this->params->get('show_qty_field', 1)) : ?>
						<td>
							<?php echo J2Store::product()->displayQuantity('com_j2store.carts', $item, $this->params, array( 'class'=>'input-mini form-control' ) ); ?>
							<a class="btn btn-small btn-danger btn-sm j2store-remove remove-icon" href="<?php echo JRoute::_('index.php?option=com_j2store&view=carts&task=remove&cartitem_id='.$item->cartitem_id); ?>">
